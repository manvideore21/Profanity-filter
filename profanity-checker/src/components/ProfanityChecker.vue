...
export default {
 ...
 methods: {
    async checkProfanity() {
      try {
        const response = await fetch('http://18.210.16.147:5000/profanity-check', {
          method: 'POST',
          headers: {
        ...
        this.result = `Result- ${data.manvi === 'No profanity Detected.'? 'No profanity Detected.' : 'Profanity Detected.'}`;
      } catch (error) {
